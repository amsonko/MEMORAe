...
class SectionEntityRepository extends EntityRepository{
     ...
     */
    public function findSectionsByPage($pageId, $language="fr"){
        $em = $this->getEntityManager();
        $query = $em->createQuery("SELECT s, m FROM MEMORAe\TextBundle\Entity\SectionEntity s LEFT JOIN s.medias m WHERE s.page = ?1 and s.language=?2")
